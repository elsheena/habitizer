...
func (h *AnalyticsHandler) GetStreaks(w http.ResponseWriter, r *http.Request) {
	...
	summary, err := h.usecase.GetStreakSummary(r.Context(), userID)
	if err != nil {
		response.Error(w, http.StatusInternalServerError, err.Error(), nil)
		return
	}

	response.JSON(w, http.StatusOK, summary, "User habit streak summary retrieved")
}
...
func (h *AnalyticsHandler) BuyFreezeBundle(w http.ResponseWriter, r *http.Request) {
	var dto domain.BuyBundleDTO
	if err := json.NewDecoder(r.Body).Decode(&dto); err != nil {
		response.Error(w, http.StatusBadRequest, "Invalid request payload", err.Error())
		return
	}

	eco, err := h.usecase.BuyStreakFreezeBundle(r.Context(), dto)
	if err != nil {
		response.Error(w, http.StatusBadRequest, err.Error(), nil)
		return
	}

	response.JSON(w, http.StatusOK, eco, "Streak freeze 3-pack bundle purchased successfully")
}
